...
import { ApiResponse } from "../../backend.nodejs/src/types";
    ...
    this.schema = Joi.object()
      .keys({
        nameStudent: Joi.string()
          ...
          .label("the field 'nameStudent'is required and only allow letters"),
        tutorial: Joi.object()
          .required()
          .keys({
            schedule: Joi.string()
        ...
        score: Joi.number()
          ...
          .max(5)
          .label
          ("The field 'score' is required and only allows numbers,between 1 to 5 and only allows numbers"),
      })
      .required();
  }
  public validate (req:Request, res:Response, next:NextFunction):any{
    ...
    if(error != null){
        const errorsValidation:any[]= []
            error.details.forEach(( detail:any )=>errorsValidation.push(detail.context.label))
            for (let i = errorsValidation.length - 1; i >= 0; i--) {
                if (errorsValidation.indexOf(errorsValidation[i]) !== i) errorsValidation.splice(i, 1);
              }
